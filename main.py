...
def TrainImages():
    ...
    message1.configure(text=res)
    message.configure(text='Total Registrations till now  : ' + str(len(np.unique(ID))))
...
def TrackImages():
    ...
    # To avoid duplicate writing in the same session
    already_marked = set()
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                name_list = df.loc[df['SERIAL NO.'] == serial]['NAME'].values
                id_list = df.loc[df['SERIAL NO.'] == serial]['ID'].values
                
                if len(name_list) > 0:
                    student_name = name_list[0]
                    student_id = id_list[0]
                else:
                    student_name = "Unknown"
                    student_id = "Unknown"

                # DISPLAY ON CAMERA SCREEN
                cv2.putText(im, str(student_name), (x, y + h), font, 1, (255, 255, 255), 2)

                # RECORD ATTENDANCE AND UPDATE UI TABLE INSTANTLY
                if student_id not in already_marked and student_id != "Unknown":
                    attendance = [str(student_id), '', str(student_name), '', str(date), '', str(timeStamp)]
                    
                    # Log to CSV
                    att_file = r"Attendance\Attendance_" + date + ".csv"
                    ...
                    # UPDATE TABLE ON MAIN SCREEN IMMEDIATELY
                    tv.insert('', 0, text=str(student_id), values=(str(student_name), str(date), str(timeStamp)))
